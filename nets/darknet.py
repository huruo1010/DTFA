...
# 将3通道RGB图通道数转为指定输出通道数量，(B, 3, H, W)->(B, 1, H/2, W/2)
...
# Spatial Pyramid Pooling：空间金字塔池化结构（yolov3结构）
class SPPBottleneck(nn.Module):
    ...
    def forward(self, x):
        # (B, C, H, W)->(B, C/2, H, W)
        x = self.conv1(x)
        # (B, C/2, H, W)->(B, C/2*(kernel_size+1), H, W)
        x = torch.cat([x] + [m(x) for m in self.m], dim=1)
        # (B, C/2*(kernel_size+1), H, W)->(B, C', H, W)
        x = self.conv2(x)
        return x
...
# Cross Stage Partial Network: 残差+模块融合
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x):
        if self.conv is not None:
            x = self.conv(x)
        b, _, w, h = x.shape
        # (B,C,W,H)->(B,C,H*W)->(1,B,C,H*W)->(H*W,B,C,1)->(H*W,B,C)
        p = x.flatten(2).unsqueeze(0).transpose(0, 3).squeeze(3)
        # (H*W,B,C)->C通道可学习再和原始的C相加(H*W,B,C)->(H*W,B,C,1)->(1,B,C,H*W)->(B,C2,W,H)
